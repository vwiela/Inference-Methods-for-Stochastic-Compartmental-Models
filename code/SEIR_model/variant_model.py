...
def _load_variant_model(N, T):
    ...
    if not _variant_model_initialized:
        if system == "Windows":
            jl_path = os.path.join(os.path.dirname(__file__), "virus_variant_est_infc_windows.jl")
        if system == "Linux":
...
def seir2v_forward(params, N0=180000, T1=400):
    ...
    E_wt, S, R_var, E_both, I_wt, I_both, E_var, R_both, R_wt, I_var = np.zeros((T1+1, 1)), np.zeros((T1+1, 1)), np.zeros((T1+1, 1)), np.zeros((T1+1, 1)), np.zeros((T1+1, 1)), np.zeros((T1+1, 1)), np.zeros((T1+1, 1)), np.zeros((T1+1, 1)), np.zeros((T1+1, 1)), np.zeros((T1+1, 1))
    for i in range(T1+1):
...
def seir2v_gaussian_dense(params, timepoints, std1, std2, T1 = 400):   
    ...
    for n in range(n_draws):
        ...
        E_wt, S, R_var, E_both, I_wt, I_both, E_var, R_both, R_wt, I_var, N_t = seir2v_forward([gamma_inv, kappa_inv, beta, t1, I0])
...
def seir2v_gaussian_sparse(params, timepoints, std1, std2, T1 = 400, config = None):   
    ...
    for n in range(n_draws):
        ...
        timepoints1 = config["timepoints1_nonmissing"]#sample(list(timepoints), number_timepoints1)        

        #number_timepoints2 = random.randint(4, len(timepoints))
        timepoints2 = config["timepoints2_nonmissing"]#sample(list(timepoints), number_timepoints2)
...
def seir2v_binomial_dense(params, timepoints, samplesize1, samplesize2, T1 = 400):   
    ...
    for n in range(n_draws):
        ...
        for i in range(len(timepoints)):
            ...
            p2 = np.clip((R_wt[t] + R_var[t] + R_both[t] + E_both[t] + I_both[t]) / N_t[t], 0.0, 1.0).item()
            n2 = samplesize2[i]
...
def seir2v_binomial(params, batch_size, config):   
    ...
    assert(
        n_params == 6 or n_params == 5
    ), "n_params of `params` needs to be 5 or 6"
...
def generative_model_seir2v(batch_size, config, prior_dict, observationtype = "binomial"):
    ...
    if (config["type"] == "dense"):
        if (observationtype == "binomial"): 
            sim_data = seir2v_binomial_dense(params, timepoints = config["timepoints"], samplesize1 = config["samplesize1"], samplesize2 = config["samplesize2"])
        if (observationtype == "gaussian" or observationtype == "normal"): 
            sim_data = seir2v_gaussian_dense(params, timepoints = config["timepoints"], std1 = config["std1"], std2 = config["std2"])
    if (config["type"] == "sparse"):
        if (observationtype == "binomial"): 
            sim_data = seir2v_binomial_sparse(params, timepoints = config["timepoints"], samplesize1 = config["samplesize1"], samplesize2 = config["samplesize2"])
        if (observationtype == "gaussian" or observationtype == "normal"): 
            sim_data = seir2v_gaussian_sparse(params, timepoints = config["timepoints"], std1 = config["std1"], std2 = config["std2"], config = config)
