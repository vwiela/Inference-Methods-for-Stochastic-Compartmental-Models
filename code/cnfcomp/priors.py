...
from .utils import unconstrain, constrain
...
def seir2v_prior_full_test():
    ...
    return np.array([gamma_inv, beta, s, t, I0])
...
def seir2v_prior_full_wrap():
    prior = Prior(prior_fun=seir2v_prior_full, param_names=[r"$\gamma$", r"$\kappa$", r"$\beta$", r"s", r"t", r"I"])
...
def seir2v_prior_full_test_wrap():
    prior = Prior(prior_fun=seir2v_prior_full_test, param_names=[r"$\gamma$", r"$\beta$", r"s", r"t", r"I"])

    params = prior.__call__(1000)["prior_draws"]
    params = seir2v_prior_full_test_transform(params)
    prior_means = np.mean(params, axis=0, keepdims=True)
    prior_stds = np.std(params, axis=0, keepdims=True)
    ...
    prior_dict = {"prior": prior,
                  "prior_means": prior_means,
                  "prior_stds": prior_stds,
                  "transform" : seir2v_prior_full_test_transform,
                  "transform_inv" : seir2v_prior_full_test_transform_inv
                }
    return prior_dict

def seir2v_prior_reparam_wrap():
    prior = Prior(prior_fun=seir2v_prior_reparam, param_names=[r"$r0$", r"$e0$", r"$s0$", r"t", r"$I0$"])
...
def seir2v_prior_full_uniform_wrap():
    prior = Prior(prior_fun=seir2v_prior_full_uniform, param_names=[r"$\gamma$", r"$\kappa$", r"$\beta$", r"s", r"t", r"I"])
...
def seir2v_prior_reparam_nonuniform_wrap():
    prior = Prior(prior_fun=seir2v_prior_reparam_nonuniform, param_names=[r"$r0$", r"$e0$", r"$s0$", r"t", r"$I0$"])
...
def sir_prior_wrap():
    prior = Prior(prior_fun=sir_prior, param_names=[r"$\beta$", r"$\gamma$"])
