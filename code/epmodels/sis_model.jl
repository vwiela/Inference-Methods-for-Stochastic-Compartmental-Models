...
function SIS_SDEProblem(parammap, N; endtime=100.0, initial_state=nothing)
    ...
    diffusion = [sqrt(β*s*i/N) -sqrt(γ*i/N) ;-sqrt(β*s*i/N) sqrt(γ*i/N)]
    
    @named sde = SDESystem(drift, diffusion, t, [s, i], [β, γ])
    if isnothing(parammap)
        parammap = [β=>0.2, γ=>0.1]
    end
...
function SIS_Model_Simulation(sdeprob, params)
    """
...
function SIS_SDE(N, T; initial_infc=1800)
    """
    Function to create the SIS model.
    ...
    sdeprob = SIS_SDEProblem(nothing, N, endtime=endtime, initial_state=initial_state)

    return params -> SIS_Model_Simulation(sdeprob, params)
end

function SIS_SDE_Ensemble(N, T; initial_infc=1800)
    """
    Function to create the SIS model.
    - params: parameter vector of the model in the ordering 
        [beta, gamma]
    ...
    return param_list -> SIS_Model_Ensemble_Simulation(sdeprob, param_list)
end

function  SIS_Model_Ensemble_Simulation(sdeprob, param_list)
    """
